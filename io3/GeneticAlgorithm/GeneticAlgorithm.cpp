...
Individual GeneticAlgorithm::run(ActivationFunction* fun){
	try{
		CheckSettings();
	}
	...
	while(noOfEpochs != settings.maxNoOfEpochs && generationsWithoutImprovement != 5){
		population.checkFitnessScores(testSet, trainingSet.getVectorOfFeatures().size(), settings, fun);
		...
		children.checkFitnessScores(testSet, trainingSet.getVectorOfFeatures().size(), settings, fun);
		children.setStats();
...
bool GeneticAlgorithm::isEmpty(fstream &resultFile) const{ return (bool)resultFile.tellg() == 0; }
